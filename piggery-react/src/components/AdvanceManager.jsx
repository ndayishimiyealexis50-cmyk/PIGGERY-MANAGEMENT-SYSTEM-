/**
 * AdvanceManager.jsx
 * Migrated from index.html §14 ADVANCE MANAGER
 *
 * Props:
 *   user, users, advances, setAdvances,
 *   salaries, setSalaries, expenses, setExpenses,
 *   capital, setCapital
 *
 * External deps:
 *   fsSet, uid, toDay, isAdminUser, fmtRWF, capitalTx
 */
...
function statusBadge(s) {
  const styles = {
    pending:  { bg: "rgba(245,158,11,.1)",  color: "#d97706", border: "rgba(245,158,11,.3)",  label: "⏳ Pending" },
    approved: { bg: "rgba(22,163,74,.1)",   color: "#16a34a", border: "rgba(22,163,74,.3)",   label: "✅ Approved" },
    rejected: { bg: "rgba(239,68,68,.1)",   color: "#dc2626", border: "rgba(239,68,68,.3)",   label: "❌ Rejected" },
  };
  const st = styles[s] || styles.pending;
  return <span style={{ background: st.bg, color: st.color, border: "1px solid " + st.border, padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 700 }}>{st.label}</span>;
}

/* ─── Admin card ──────────────────────────────────────────────── */
function AdvanceAdminCard({ adv, onApprove, onReject, onDelete }) {
  const [rejecting, setRejecting] = useState(false);
  const [reason, setReason]       = useState("");
  const borderCol = adv.status === "approved" ? "#16a34a" : adv.status === "rejected" ? "#dc2626" : "#d97706";

  return (
    <div style={{ ...S.card, marginBottom: 10, borderLeft: "4px solid " + borderCol }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10 }}>
        <div style={{ flex: 1 }}>
          ...
          {adv.status === "approved" && <div style={{ fontSize: 11, color: "#16a34a", marginTop: 4 }}>✅ Approved {adv.approvedAt?.slice(0, 10)} by {adv.approvedBy}</div>}
          {adv.status === "rejected" && <div style={{ fontSize: 11, color: C.red, marginTop: 4 }}>Reason: {adv.rejectionReason}</div>}
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 7, flexShrink: 0 }}>
          {adv.status === "pending" && (
            <>
              <button onClick={onApprove} style={{ ...S.btn("#16a34a"), fontSize: 12, padding: "7px 16px", borderRadius: 9 }}>✅ Approve</button>
          ...
          {onDelete && <button onClick={onDelete} style={{ fontSize: 10, padding: "4px 10px", borderRadius: 6, border: "1px solid rgba(239,68,68,.3)", background: "transparent", color: C.red, cursor: "pointer", fontFamily: "inherit" }}>🗑️ Delete</button>}
        </div>
      </div>

      {rejecting && (
        <div style={{ marginTop: 12, padding: "12px 14px", background: "rgba(239,68,68,.05)", border: "1px solid rgba(239,68,68,.2)", borderRadius: 9 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: C.red, marginBottom: 8 }}>Reason for rejection (optional)</div>
          <input value={reason} onChange={e => setReason(e.target.value)} placeholder="e.g. Advance limit reached…" style={{ ...S.inp, marginBottom: 8 }} />
...
export default function AdvanceManager({ user, users, advances, setAdvances, salaries, setSalaries, expenses, setExpenses, capital, setCapital }) {
  ...
  async function submitRequest() {
    ...
    const rec = {
      ...
      amount: parseFloat(form.amount), reason: form.reason, notes: form.notes,
      repayMonth: parseInt(form.repayMonth), repayYear: parseInt(form.repayYear),
      status: "pending", requestedAt: new Date().toISOString(),
    };
    const updated = [...(advances || []), rec];
    setAdvances(updated);
    try { await fsSet("advances", updated); } catch {}
    showToast("✅ Advance request submitted! Admin will review it shortly.");
    window._addAuditLog?.("add", `Advance requested: ${fmtRWF(parseFloat(form.amount))} — ${form.reason}`);
    setForm(defForm); setTab("mylist"); setSaving(false);
  }
  ...
  async function approve(id) {
    ...
    try { await fsSet("advances", updatedAdv); } catch {}
    const alreadyRecorded = (expenses || []).some(e => e.refAdvanceId === id);
    if (!alreadyRecorded) {
      const desc = `Salary Advance — ${adv.workerName} (${adv.reason})`;
      const expRec = { id: uid(), workerId: user.uid || user.id, worker: user.name, category: "Salary Advance", description: desc, amount: adv.amount, date: approvedAt, source: "advance_approval_" + id, refAdvanceId: id, approved: true };
      const updatedExp = [...(expenses || []), expRec];
      if (setExpenses) setExpenses(updatedExp);
      try { await fsSet("expenses", updatedExp); } catch {}
      if (capital && setCapital) capitalTx(capital, setCapital, { type: "expense", category: "Salary Advance", amount: adv.amount, description: desc, date: approvedAt, refId: "advance_" + id });
    }
    showToast(`✅ Advance of ${fmtRWF(adv.amount)} approved for ${adv.workerName}`);
  ...
  async function reject(id, reason) {
    ...
    window._addAuditLog?.("reject", `Advance rejected for ${adv.workerName}`);
  }
      ...
      {!isAdmin && tab === "mylist" && (
        <div>
          ...
          {myAdvances.slice().reverse().map(a => (
            <div key={a.id} style={{ ...S.card, marginBottom: 10, borderLeft: "4px solid " + (a.status === "approved" ? "#16a34a" : a.status === "rejected" ? C.red : C.amber) }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 8 }}>
                <div style={{ flex: 1 }}>
                  ...
                  {a.status === "approved" && <div style={{ fontSize: 11, color: "#16a34a", marginTop: 4 }}>✅ Approved on {a.approvedAt?.slice(0, 10)}</div>}
                  {a.status === "rejected" && <div style={{ fontSize: 11, color: C.red, marginTop: 4 }}>❌ {a.rejectionReason || "Not approved"}</div>}
                </div>
              </div>
            </div>
          ))}
      ...
      {isAdmin && (
        <div>
          ...
          {listToShow.slice().reverse().map(a => (
            <AdvanceAdminCard key={a.id} adv={a}
              onApprove={() => approve(a.id)}
              onReject={(r) => reject(a.id, r)}
              onDelete={() => {
                if (!window.confirm("Delete this advance record?")) return;
                const updated = (advances || []).filter(x => x.id !== a.id);
                setAdvances(updated); fsSet("advances", updated);
                window._addAuditLog?.("delete", `Advance deleted: ${a.workerName} ${fmtRWF(a.amount)}`);
              }}
            />
          ))}
        </div>
