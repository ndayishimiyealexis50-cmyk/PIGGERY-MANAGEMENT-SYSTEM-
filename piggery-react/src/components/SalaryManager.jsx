/**
 * SalaryManager.jsx
 * Migrated from index.html §14d SALARY MANAGER (main payroll component)
 *
 * Props:
 *   user, users, salaries, setSalaries, expenses, setExpenses,
 *   capital, setCapital, bonusRequests, setBonusRequests,
 *   salaryConfigs, setSalaryConfigs, advances
 *
 * External deps:
 *   fsSet(key, value)
 *   toDay()
 ...
 *   isAdminUser(user)
 *   fmtRWF(n)
 *   capitalTx(capital, setCapital, txObj)
...
const fsSet  = async () => {};
const capitalTx = () => {};
const _db = null; // import { db as _db } from "../firebase";
// ────────────────────────────────────────────────────────────────
...
function calcNetPay(gross, deductions = [], bonuses = []) {
  ...
  const hasAdvance  = deductions.some(d => d.type === "Salary Advance Deduction");
  const effectiveDeduct = hasAdvance ? Math.max(0, totalDeduct - totalBonus) : totalDeduct;
  const effectiveBonus  = hasAdvance ? Math.max(0, totalBonus - totalDeduct)  : totalBonus;
  return {
    gross: parseFloat(gross) || 0, totalBonus, totalDeduct,
    subtotal: (parseFloat(gross) || 0) + effectiveBonus,
    net: Math.max(0, (parseFloat(gross) || 0) + effectiveBonus - effectiveDeduct),
  };
}
...
function PayslipCard({ sal, onClose }) {
  ...
  const statusEl = () => {
    if (sal.status === "paid") return <span style={{ padding: "3px 10px", borderRadius: 20, background: "rgba(22,163,74,.1)", color: C.accent, fontSize: 11, fontWeight: 700 }}>✅ Paid</span>;
    return <span style={{ padding: "3px 10px", borderRadius: 20, background: "rgba(245,158,11,.1)", color: C.amber, fontSize: 11, fontWeight: 700 }}>⏳ Pending</span>;
  };
  const periodLabel = () => `${MONTHS_LBL[(sal.month || 1) - 1]} ${sal.year || ""}`;
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.45)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}
      ...
      <div style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 440, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 24px 60px rgba(0,0,0,.25)" }}>
        <div style={{ background: "linear-gradient(145deg,#071410 0%,#0e2213 55%,#102616 100%)", padding: "20px 22px", borderRadius: "16px 16px 0 0", position: "relative" }}>
          <div style={{ fontSize: 11, color: "rgba(74,222,128,.7)", fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", marginBottom: 4 }}>Alexis Gold Piggery · Official Payslip</div>
          <div style={{ fontSize: 17, fontWeight: 800, color: "#fff" }}>👷 {sal.workerName}</div>
          <div style={{ fontSize: 12, color: "rgba(255,255,255,.55)", marginTop: 2 }}>{sal.period} · {periodLabel()} · {sal.method}</div>
          <div style={{ position: "absolute", top: 18, right: 18 }}>{statusEl()}</div>
        </div>
        ...
        <div style={{ padding: "18px 22px" }}>
          ...
          {bonuses.length > 0 && (
            <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 14px", background: "rgba(22,163,74,.08)", borderRadius: 8, marginBottom: 8, border: "1px solid rgba(22,163,74,.2)" }}>
        ...
        <div style={{ padding: "0 22px 18px" }}>
          <button onClick={onClose} style={{ width: "100%", padding: "11px", borderRadius: 10, border: "1px solid #e2e8f0", background: "#f8fafc", color: "#475569", fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>
            Close Payslip
          </button>
        </div>
      </div>
...
export default function SalaryManager({ user, users, salaries, setSalaries, expenses, setExpenses, capital, setCapital, bonusRequests, setBonusRequests, salaryConfigs, setSalaryConfigs, advances }) {
  ...
  function getWorkerConfig(wid) { return (salaryConfigs || []).find(c => c.workerId === wid) || null; }
  ...
  function statusEl(s) {
    ...
    if (s.status === "paid") return <span style={{ padding: "3px 10px", borderRadius: 20, background: "rgba(22,163,74,.1)", color: C.accent, fontSize: 11, fontWeight: 700 }}>✅ Paid</span>;
    if (isOvr)               return <span style={{ padding: "3px 10px", borderRadius: 20, background: "rgba(239,68,68,.1)", color: C.red,   fontSize: 11, fontWeight: 700 }}>⚠️ Overdue</span>;
    return                          <span style={{ padding: "3px 10px", borderRadius: 20, background: "rgba(245,158,11,.1)", color: C.amber, fontSize: 11, fontWeight: 700 }}>⏳ Pending</span>;
  }

  async function saveConfig() {
    if (!configForm.workerId || !configForm.grossSalary || parseFloat(configForm.grossSalary) <= 0) { showToast("Select a worker and enter their monthly gross salary.", "error"); return; }
    ...
    let updated;
    if (existing) {
      updated = (salaryConfigs || []).map(c => c.workerId === configForm.workerId ? { ...c, grossSalary: parseFloat(configForm.grossSalary), notes: configForm.notes, updatedAt: new Date().toISOString(), updatedBy: user.name } : c);
    } else {
      updated = [...(salaryConfigs || []), { id: uid(), workerId: configForm.workerId, workerName: worker?.name || "", grossSalary: parseFloat(configForm.grossSalary), notes: configForm.notes, createdAt: new Date().toISOString(), createdBy: user.name, updatedAt: new Date().toISOString(), updatedBy: user.name }];
    }
  ...
  function handleWorkerChange(wid) {
    ...
    setForm(f => ({ ...f, workerId: wid, workerName: w?.name || "", amount: config ? String(config.grossSalary) : "", deductions: autoDeductions, bonuses: autoBonuses }));
  }
  ...
  async function scheduleSalary() {
    ...
    const rec = {
      id: uid(), workerId: form.workerId, workerName: form.workerName,
      gross, deductions: form.deductions.map(d => ({ ...d, amount: parseFloat(d.amount) || 0 })),
  ...
  async function paySalary(sal) {
    const alreadyPaid = (salaries || []).some(s => s.id !== sal.id && s.workerId === sal.workerId && s.month === sal.month && s.year === sal.year && s.status === "paid");
    if (alreadyPaid) { showToast(`⛔ ${sal.workerName} has already been paid for ${MONTHS_LBL[(sal.month || 1) - 1]} ${sal.year}.`, "error"); setConfirmId(null); return; }
    setSaving(true);
    const paidAt  = toDay();
    ...
    const alreadyRecorded = (expenses || []).some(e => e.refSalaryId === sal.id);
    if (!alreadyRecorded) {
    ...
    showToast(`💸 ${fmtRWF(netAmt)} paid to ${sal.workerName}`);
    window._addAuditLog?.("edit", `Salary paid: ${fmtRWF(netAmt)} to ${sal.workerName}`);
  ...
  async function deleteSalary(salId) {
    ...
    await fsSet("salaries", updatedSal);
  ...
  return (
    <div className="fade-in">
      ...
      {/* Banner */}
      <div style={{ background: "linear-gradient(135deg,#0c1f18 0%,#122d20 55%,#0e2218 100%)", borderRadius: 14, padding: "22px 24px", marginBottom: 18, boxShadow: "0 8px 28px rgba(0,0,0,.2)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10 }}>
          <div>
            <div style={{ fontSize: 18, fontWeight: 800, color: "#fff", marginBottom: 3 }}>💼 Salary &amp; Payments</div>
            <div style={{ fontSize: 12, color: "rgba(255,255,255,.55)" }}>{isAdmin ? "Manage salaries · deductions · bonuses" : "Your personal salary records"}</div>
          </div>
      ...
      {isAdmin && pendingApprovalList.length > 0 && (
        <div style={{ background: "linear-gradient(135deg,rgba(245,158,11,.08),rgba(234,179,8,.04))", border: "2px solid rgba(245,158,11,.35)", borderRadius: 12, padding: "14px 18px", marginBottom: 14, display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: "#b45309" }}>Workers: <strong>{pendingApprovalList.map(s => s.workerName).join(", ")}</strong></div>
          </div>
      ...
      {tab === "config" && isAdmin && (
        <div>
          <div style={{ ...S.card, border: "1px solid rgba(22,163,74,.18)" }}>
            ...
            <div style={{ fontSize: 12, color: C.faint, marginBottom: 18, lineHeight: 1.6 }}>
              Set each worker's fixed monthly gross salary. On the 25th, salaries auto-generate with bonuses and advance deductions.
            </div>
            <div style={{ marginBottom: 12 }}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <input placeholder="e.g. Includes housing allowance" value={configForm.notes} onChange={e => setConfigForm(f => ({ ...f, notes: e.target.value }))} style={S.inp} />
            </div>
            <button onClick={saveConfig} disabled={configSaving || !configForm.workerId || !configForm.grossSalary}
              style={{ ...S.btn(), width: "100%", padding: "11px", fontSize: 14, opacity: (!configForm.workerId || !configForm.grossSalary) ? 0.5 : 1 }}>
              ...
              return (
                <div key={w.uid || w.id} style={{ ...S.row, marginBottom: 8, flexWrap: "wrap", gap: 8 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 36, height: 36, borderRadius: "50%", background: cfg ? "rgba(22,163,74,.12)" : "rgba(100,116,139,.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>👤</div>
                    <div>
                      <div style={{ fontWeight: 700, fontSize: 13, color: C.text }}>{w.name}</div>
                      <div style={{ fontSize: 11, color: C.faint }}>{w.email || ""}</div>
                    </div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    {cfg ? (
                      <>
                        ...
                        <button onClick={() => setConfigForm({ workerId: w.uid || w.id, grossSalary: String(cfg.grossSalary), notes: cfg.notes || "" })}
                          style={{ padding: "5px 11px", borderRadius: 7, border: "1px solid " + C.border, background: C.elevated, color: C.muted, fontSize: 11, cursor: "pointer", fontFamily: "inherit", fontWeight: 700 }}>✏️ Edit</button>
                      </>
      ...
      {tab === "overview" && (
        <div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(140px,1fr))", gap: 10, marginBottom: 16 }}>
            {[
              { l: "Total Paid",    v: fmtRWF(totalPaid),    c: "#16a34a", i: "✅" },
              { l: "Pending",       v: fmtRWF(totalPending),  c: "#d97706", i: "⏳" },
              { l: "Overdue",       v: fmtRWF(totalOverdue),  c: totalOverdue > 0 ? "#dc2626" : C.faint, i: "⚠️" },
              { l: "Records",       v: visibleSalaries.length, c: C.text,   i: "📋" },
            ].map(k => (
            ...
            {paidList.slice().sort((a, b) => (b.paidAt || "").localeCompare(a.paidAt || "")).slice(0, 6).map((s, i) => (
              <div key={s.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 12px", background: i % 2 === 0 ? C.elevated : "transparent", borderRadius: 8, marginBottom: 4, cursor: "pointer" }} onClick={() => setViewPayslip(s.id)}>
                <div>
                  ...
                  <div style={{ fontSize: 11, color: C.faint }}>{periodLabel(s)} · Paid {s.paidAt || "—"}</div>
                </div>
                <div style={{ textAlign: "right" }}>
      ...
      {tab === "schedule" && isAdmin && (
        <div style={{ maxWidth: 560 }}>
          <div style={{ ...S.card, border: "1px solid rgba(22,163,74,.2)" }}>
            ...
            <div style={{ fontSize: 12, color: C.faint, marginBottom: 16 }}>{editId ? "Update salary, deductions or bonuses." : "Create a salary with deductions/bonuses — worker sees full payslip."}</div>
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={S.lbl}>Notes (optional)</label>
              <input placeholder="e.g. Performance bonus included…" value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} style={S.inp} />
            </div>
            ...
            {form.amount && parseFloat(form.amount) > 0 && (
              <div style={{ padding: "14px 16px", background: "linear-gradient(135deg,rgba(22,163,74,.06),rgba(16,185,129,.03))", border: "1.5px solid rgba(22,163,74,.2)", borderRadius: 10, marginBottom: 14 }}>
                ...
                {[["Base Salary", fmtRWF(preview.gross), C.text], ...(preview.totalBonus > 0 ? [["+ Total Bonuses", "+ " + fmtRWF(preview.totalBonus), C.accent]] : []), ...(preview.totalDeduct > 0 ? [["− Total Deductions", "− " + fmtRWF(preview.totalDeduct), C.red]] : [])].map(([l, v, c]) => (
                  <div key={l} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: C.muted, marginBottom: 3 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={editId ? null : scheduleSalary} disabled={saving || !form.workerId || !form.amount || parseFloat(form.amount) <= 0}
      ...
      {tab === "history" && (
        <div>
          ...
          {visibleSalaries.slice().sort((a, b) => (b.paidAt || b.dueDate || b.createdAt || "").localeCompare(a.paidAt || a.dueDate || a.createdAt || "")).map(s => {
            ...
            return (
              <div key={s.id} style={{ ...S.card, borderLeft: "4px solid " + (needsApproval ? "#f59e0b" : borderCol), marginBottom: 10, padding: "13px 16px", background: needsApproval ? "rgba(245,158,11,.02)" : "#fff" }}>
                {needsApproval && <div style={{ background: "rgba(245,158,11,.1)", border: "1px solid rgba(245,158,11,.3)", borderRadius: 7, padding: "6px 12px", marginBottom: 8, fontSize: 11, color: "#92400e", fontWeight: 600 }}>🔔 Auto-generated — awaiting your approval</div>}
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 8 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "auto 1fr", columnGap: 12, rowGap: 2, fontSize: 11, color: C.faint }}>
                      ...
                      {s.paidAt && <><span>Paid:</span><span style={{ color: C.accent, fontWeight: 600 }}>{s.paidAt}</span></>}
                    </div>
                  </div>
      ...
      {tab === "byworker" && isAdmin && (
        <div>
          ...
          {Object.entries(byWorker).sort((a, b) => b[1].totalNet - a[1].totalNet).map(([wid, data]) => (
            <div key={wid} style={{ ...S.card, marginBottom: 12 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                {data.records.slice(-5).map(r => (
                  <button key={r.id} onClick={() => setViewPayslip(r.id)} style={{ padding: "4px 10px", borderRadius: 12, background: r.status === "paid" ? "rgba(22,163,74,.1)" : "rgba(245,158,11,.1)", border: "1px solid " + (r.status === "paid" ? "rgba(22,163,74,.25)" : "rgba(245,158,11,.25)"), fontSize: 10, color: r.status === "paid" ? C.accent : C.amber, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
      ...
      {confirmId && (() => {
        ...
        if (!sal) return null;
        return (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.45)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(6px)" }}
