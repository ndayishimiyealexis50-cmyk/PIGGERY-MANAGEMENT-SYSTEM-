...
import { fmtRWF, fsSet, isAdminUser } from '../utils/helpers';
...
export default function SaleLog({ sales, setSales, pigs, feeds, logs, expenses, incomes, allData, user }) {
  ...
  return (
    <div>
      ...
      <AIPrediction pigs={pigs} feeds={feeds} sales={sales} logs={logs} expenses={expenses} incomes={incomes} topic="Best sell timing, optimal weight at sale, market forecast, best buyers." label="Sales & Market Forecast" icon="🏷️" />
    </div>
  );
}
