...
// ── Firestore reference ──
const farmRef = doc(db, 'farmiq', 'farm');

// ── Helper: persist pigs array to Firestore ──
function savePigs(updatedPigs) {
  updateDoc(farmRef, { pigs: updatedPigs }).catch(e => console.error("Firestore savePigs error:", e));
}
      ...
      {editPigId === p.id && editForm ? (
        ...
        <>
          ...
          <div style={{ display:"flex", gap:5, marginBottom:6 }}>
            ...
            <button
              onClick={() => setPigs(prev => {
                const updated = prev.map(pig => pig.id===p.id ? {...pig, status:pig.status==="active"?"sold":"active"} : pig);
                savePigs(updated);
                return updated;
              })}
...
export default function Pigs({ pigs, setPigs, logs, allData, capital, setCapital }) {
  ...
  function deletePig(pig) {
    ...
    setPigs(prev => {
      const updated = prev.filter(p => p.id !== pig.id);
      savePigs(updated);
      return updated;
    });
  ...
  function saveGrowth(pigId, updates) {
    setPigs(prev => {
      const updated = prev.map(p => p.id===pigId ? {...p,...updates} : p);
      savePigs(updated);
      return updated;
    });
    setGrowthPig(null);
  ...
  function saveEdit() {
    if (!editForm) return;
    setPigs(prev => {
      const updated = prev.map(p => p.id===editPigId ? {...p,...editForm, weight:parseFloat(editForm.weight)||p.weight, length:parseFloat(editForm.length)||p.length} : p);
      savePigs(updated);
      return updated;
    });
          ...
          <div style={{ background:"#fff", borderRadius:20, padding:22, width:270, boxShadow:"0 24px 60px rgba(0,0,0,.28)" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {[
                ...
                { icon:"🏷️", label:"Mark as Sold", color:C.amber||"#f59e0b", action:() => { setPigs(prev => { const u=prev.map(pig=>pig.id===longPressedPig.id?{...pig,status:"sold"}:pig); savePigs(u); return u; }); setLongPressedPig(null); } },
