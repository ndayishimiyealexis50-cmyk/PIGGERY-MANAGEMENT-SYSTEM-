...
import { fmtRWF, fmtNum, fsSet, isAdminUser } from '../utils/helpers';
...
export default function FeedLog({ feeds, setFeeds, pigs, logs, sales, expenses, incomes, allData, user }) {
  ...
  return (
    <div>
      ...
      <AIPrediction pigs={pigs} feeds={feeds} sales={sales} logs={logs} expenses={expenses} incomes={incomes} topic="Optimal feed per pig stage, cost savings, weight gain forecast, best suppliers." label="Feed Optimization" icon="🌾" />
    </div>
  );
}
