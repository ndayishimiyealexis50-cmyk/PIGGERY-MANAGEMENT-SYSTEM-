...
import { setDoc, collection, onSnapshot, orderBy, query } from "firebase/firestore";
...
export default function AHome({
  ...
  // ── Live market survey listener ──────────────────────────────────────────
  useEffect(() => {
    const q = query(collection(db, "marketSurveys"), orderBy("date", "desc"));
    return onSnapshot(q, snap => setLiveMarket(snap.docs.map(d => d.data())));
  }, []);
  ...
  // ── Herd value from live Firestore market surveys ─────────────────────────
  const latestLivePig   = liveMarket.find(s => s.category === "live_pig");
  const hasSurveys      = !!latestLivePig;
  const herdValue       = hasSurveys
    ? active.reduce((sum, pig) => sum + (pig.weight || 0) * latestLivePig.price, 0)
    : 0;
    ...
    <div className="fade-in">
      ...
      <div style={{ background: "linear-gradient(145deg,#071410 0%,#0e2213 55%,#102616 100%)", borderRadius: 14, padding: "18px 22px", marginBottom: 14, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10, boxShadow: "0 8px 28px rgba(0,0,0,.2)", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button onClick={() => { setShowCapitalSet(!showCapitalSet); setNewInitial(String(safeCapital.initial || "")); }}
          ...
          <button onClick={fixCapital} disabled={fixing}
      ...
      {/* ── Mismatch Alert ── */}
      {hasMismatch && (
        <div style={{ padding: "14px 16px", background: "rgba(239,68,68,.1)", border: "2px solid rgba(239,68,68,.4)", borderRadius: 12, marginBottom: 12 }}>
        ...
        <div style={{ ...S.card, marginBottom: 12, border: "1px solid rgba(74,222,128,.3)" }}>
          ...
          <div style={{ display: "flex", gap: 9 }}>
            <input type="number" value={newInitial} onChange={e => setNewInitial(e.target.value)}
              placeholder="e.g. 500000" style={{ ...S.inp, flex: 1 }} />
            <button onClick={() => { setCapital(prev => ({ ...prev, initial: parseFloat(newInitial) || 0 })); setShowCapitalSet(false); }}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(140px,1fr))", gap: 12, marginBottom: 16 }}>
        {[
          ...
          { icon:"💎", label:"Herd Value",     value: hasSurveys ? fmtRWF(herdValue) : "No survey", sub: hasSurveys ? `@ RWF ${latestLivePig.price.toLocaleString()}/kg` : "Add a market survey", color: C.purple, bg:"rgba(124,58,237,.06)", border:"rgba(124,58,237,.18)" },
      ...
      <div style={S.g2}>
        ...
        <div style={S.card}>
          ...
          {[
            ...
            ["Herd Market Value", hasSurveys ? fmtRWF(herdValue) : "No survey",                                  C.purple                       ],
            ["Total Portfolio",   hasSurveys ? fmtRWF(profit + herdValue) : "—",                                 profit + herdValue >= 0 ? C.accent : C.red ],
